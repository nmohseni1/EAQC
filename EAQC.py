...
parser.add_argument('--N', metavar=('n'), type=int, help='number of particles')
parser.add_argument('--tau', metavar=('tau'), type=int, help='evolution time')
...
count_time=0
mat1 = genfromtxt("Series 3");

count_time=count_time+1
print('taulist',taumax)
taulist= np.linspace(0,taumax,100)
num_trial=800                     #number of instances
p1_final=[]
...
for i in range(1,num_trial+1):
    ...
    evals1, ekets1=H11.eigenstates() # Required for Calculating the number of Replica states
    exx_1=np.zeros((1,M))
    exx_2=np.zeros((1,M))
    exx_3=np.zeros((1,M))
...
#**************************************** solving the master equation without and with decoherence(Forth argument is related to deohernce operators)
        ...
        mesolve(h_t, psii, taulist, [np.sqrt(0.0001)*sz1,np.sqrt(0.0001)*sz2,np.sqrt(0.0001)*sz3], process_rho, args,_safe_mode=False)
...
#print('sizesuccess_eachtrial',size(success_eachtrial))
success_alltrials=sum(success_eachtrial[i] for i in range(64))
#print('success_alltrials',success_alltrials)
p1_final.append(success_alltrials/64)
